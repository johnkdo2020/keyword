...
from keywords.exceptions import RelatedAPIErrorException, RelatedAPIBackendProblemException, \
    RelatedAPINaverAPIProblemException
from keywords.models import Keyword
from keywords.serializers import KeywordSerializer
from utils.naver_related_funtion import rel_keyword_api
...
class RelatedKeywordViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        response = rel_keyword_api(request_keyword)
        status_code = response.status_code
        status_error_code = status_code // 100
        if status_code == 200:
            try:
                data = {"relatedKeywordList": response.json()["keywordList"][:100]}
            except Exception as e:
                raise RelatedAPINaverAPIProblemException
        elif status_error_code is 4:
            raise RelatedAPIErrorException
        elif status_error_code is 5:
            raise RelatedAPIBackendProblemException
        else:
            data = response.json()
